...
from rendering import ROOT, OverlayRenderer
...
def formats():
    ...
    return [key for key,encoder in [('mp4','libx264'),('prores','prores_ks'),('webm','libvpx-vp9')] if encoder in result.stdout]
...
class Job:
    def __init__(self,activity,cfg,directory=None):
        ...
        self.path=self.directory/('overlay-'+self.id[:10]+{'mp4':'.mp4','prores':'.mov','webm':'.webm'}[cfg['format']])
    ...
    def run(self):
        ...
        try:
            renderer=OverlayRenderer(self.activity,self.cfg)
            w,h=renderer.size
            ...
            if self.cfg['format']=='mp4': args+=['-c:v','libx264','-preset','veryfast','-crf','18','-pix_fmt','yuv420p','-movflags','+faststart']
            elif self.cfg['format']=='prores': args+=['-c:v','prores_ks','-profile:v','4','-pix_fmt','yuva444p10le','-alpha_bits','16']
            ...
            with log.open('wb') as errors:
                ...
                for i in range(self.total):
                    ...
                    frame=renderer.frame(self.cfg['start']+i/self.cfg['fps'])
